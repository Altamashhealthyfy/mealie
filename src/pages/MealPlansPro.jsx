...
## UPLOADED MEDICAL REPORT DATA (PRIORITY - USE THIS TO OVERRIDE/SUPPLEMENT INTAKE DATA):
Additional Health Conditions from Report: ${reportData.health_conditions?.join(', ') || 'None'}
Lab Values from Report: ${Object.entries(reportData.lab_values || {}).map(([k, v]) => k + ': ' + v).join(', ') || 'None'}
Medications from Report: ${reportData.medications?.join(', ') || 'None'}
Stage/Severity from Report: ${reportData.stage_severity || 'Not specified'}
Key Clinical Findings: ${reportData.key_findings?.join('; ') || 'None'}
Dietary Recommendations from Report: ${reportData.dietary_recommendations?.join('; ') || 'None'}
IMPORTANT: Incorporate these medical report findings into the meal plan. If report data conflicts with intake data, the report takes priority.
` : '';
