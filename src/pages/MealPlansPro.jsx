...
export default function MealPlansPro() {
  ...
  const { data: proMealPlans } = useQuery({
    ...
    queryFn: async () => {
      ...
      // super_admin sees all plans; everyone else sees only their own
      if (user?.user_type === 'super_admin') return advancedPlans;
                  ...
                  <CardContent>
                    <div className="relative">
                      <Input
                        ...
                        value={selectedClientId ? (clients.find(c => c.id === selectedClientId)?.full_name || clientSearchTemplate) : clientSearchTemplate}
                        onChange={(e) => {
                          setClientSearchTemplate(e.target.value);
                      ...
                      {selectedClientId && (
                        <button
                          onClick={() => { setSelectedClientId(null); setClientSearchTemplate(''); }}
                          className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-red-500"
                        >
                          <X className="w-4 h-4" />
                        </button>
                      )}
                      {!selectedClientId && clientSearchTemplate && (
                        <div className="absolute z-10 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg max-h-48 overflow-y-auto">
                          {clients.filter(c => c.full_name.toLowerCase().includes(clientSearchTemplate.toLowerCase())).map(client => (
                            <button
                              key={client.id}
                              className="w-full text-left px-4 py-2.5 hover:bg-purple-50 flex items-center gap-2"
                              onClick={() => { setSelectedClientId(client.id); setClientSearchTemplate(''); }}
                            >
                              <span className="font-medium">{client.full_name}</span>
                              <Badge variant="outline" className="text-xs capitalize">{client.food_preference}</Badge>
                            </button>
                          ))}
                          {clients.filter(c => c.full_name.toLowerCase().includes(clientSearchTemplate.toLowerCase())).length === 0 && (
                ...
                <div className="p-4 md:p-6 space-y-5 bg-white">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      <Input
                        placeholder="Search client by name..."
                        value={selectedClientId ? (clients.find(c => c.id === selectedClientId)?.full_name || clientSearch) : clientSearch}
                        onChange={(e) => {
                          ...
                          {clients
                            ...
                            .map(client => (
                              <button
                                key={client.id}
                                className="w-full text-left px-4 py-2.5 hover:bg-purple-50 flex items-center gap-2"
                                onClick={() => { setSelectedClientId(client.id); setClientSearch(''); }}
                              >
                                <span className="font-medium">{client.full_name}</span>
