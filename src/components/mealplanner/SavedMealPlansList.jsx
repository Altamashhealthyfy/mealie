...
import { Calendar, Eye, Edit, Trash2, CheckCircle, FileDown, Star } from "lucide-react";
import { format } from "date-fns";
import { createPageUrl } from "@/utils";
import AdvancedPlanPDFExport from "./AdvancedPlanPDFExport";

export default function SavedMealPlansList({ mealPlans, clients, user, onView, onEdit, onDelete, isDeleting, onBrowseTemplates }) {
    ...
    <div className="space-y-4">
              ...
              <CardContent>
                <div className="flex flex-col sm:flex-row gap-2">
                  ...
                  <Button
                    ...
                    onClick={() => setPdfPlan({ ...plan, client_name: planClient?.full_name || '' })}
                  >
      ...
      {pdfPlan && (
        <AdvancedPlanPDFExport
          plan={pdfPlan}
          clientName={pdfPlan.client_name}
          open={!!pdfPlan}
          onOpenChange={(open) => { if (!open) setPdfPlan(null); }}
        />
      )}
    </div>
  );
